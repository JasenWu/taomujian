...
<script language="javascript" >
 function check_add(){
    if(document.editForm.link_name.value==""){alert("请填写链接名称");document.editForm.link_name.focus();return false;}
	return true;
 }
</script>
...
<table width="100%"  border="1" style="border:#6298E1 tthin solid;" cellpadding="3" cellspacing="1" class="input">
  <form name="editForm" method="post" action="LinksEdit.php?Action=SaveEdit&Result=<?php echo $Result ?>&ID=<?php echo $ID ?>"  onSubmit="return check_add();" >
    <tr>
...
function NewsEdit($ID){
   ...
   if($Action=="SaveEdit"){  //添加修改数据
	 ...
	 echo "<script>alert('成功编辑链接信息！');location.href='LinksList.php';</script>";
  }else{   //提取信息
     if($Result=="Modify"){
