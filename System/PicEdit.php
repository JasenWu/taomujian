...
NewsEdit($ID) ;
if(!isset($px)){
  $px=$db->getOne("select max(views) as orders from jp_news where sortpath like '%0,20,%' ");
  $px++;	
}
if(!isset($NewsNameSi)){$NewsNameSi="";}
if(!isset($author)){$author="";}
if(!isset($type1)){$type1="";}
if(!isset($type2)){$type2="t0";}
...
<table width="100%"  border="1" style="border:#6298E1 tthin solid;" cellpadding="3" cellspacing="1" class="input">
  <form name="editForm" method="post" action="PicEdit.php?Action=SaveEdit&Result=<?php echo $Result ?>&ID=<?php echo $ID ?>"  onSubmit="return check_add();" >
    <tr>
      <td height="24" nowrap ><table width="100%" border="0" cellpadding="0" cellspacing="0"   idth="100%">
          ...
          <tr>
            ...
            <td><?php  show_select(20,$SortIDs) ;?>
            </td>
          </tr>
          <tr>
            <td height="20" align="right">Banner图片：</td>
...
function NewsEdit($ID){
   ...
   if($Action=="SaveEdit"){  //添加修改数据
	 ...
	 echo "<script>alert('成功编辑Banner信息！');location.href='PicList.php';</script>";
  }else{   //提取信息
     if($Result=="Modify"){
