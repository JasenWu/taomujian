...
<table width="100%"  border="1" style="border:#6298E1 tthin solid;" cellpadding="3" cellspacing="1" class="input">
  <form name="editForm" method="post" action="PicEdit2.php?Action=SaveEdit&Result=<?php echo $Result ?>&ID=<?php echo $ID ?>"  onSubmit="return check_add();" >
    <tr>
      <td height="24" nowrap ><table width="100%" border="0" cellpadding="0" cellspacing="0"   idth="100%">
          ...
          <tr>
            <td height="20" align="right">证书名称(中)：</td>
          ...
          <tr>
            <td height="20" align="right">证书名称(英)：</td>
          ...
          <tr>
            ...
            <td><?php  show_select(3,$SortIDs) ;?>
            </td>
          </tr>
          <tr>
            <td height="20" align="right">证书图片：</td>
          ...
          <tr>
            <td height="20" align="right">证书类型：</td>
            <td colspan="5">
...
function NewsEdit($ID){
   ...
   if($Action=="SaveEdit"){  //添加修改数据
	 ...
	 echo "<script>alert('成功编辑证书信息！');location.href='PicList2.php';</script>";
  }else{   //提取信息
     if($Result=="Modify"){
