    ...
    <td height="36" align="left" nowrap  bgcolor="#EBF2F9"  style="padding-left:20px;"><table width="86%" border="0" cellspacing="0">
      <tr>
        <form name="formSearch" method="get" action="?Result=Search">
          <td width="52%" nowrap> 留言检索：关键字：<input name="Keyword" type="text" class="textfield" value="<?php echo  $Keyword ?>" size="10">
...
<table width="93%" border="0" cellpadding="3" cellspacing="1" class="input">
  <form action="DelContent.php?Result=jp_message" method="post" name="formDel" >
    <tr >
      <td width="55" nowrap ><strong>ID</strong> </td>
      <td width="60%"  ><strong>公司名称</strong> </td>
     <!-- <td width="67" nowrap ><strong>联系人</strong> </td>-->
...
<?php 
function MessageList($Keyword){
   global $page,$pageSize,$num ;
   @$page=$_GET["page"];
   if((!$page)|| $page<1) $page=1;
   $pageSize=10;
   $firstcount = ($page -1) * $pageSize;
   $sql="select * from jp_message  order by id desc " ;
   if($Keyword){$sql="select * from jp_message where  title like '%$Keyword%' or content like '%$Keyword%' or type1 like '%$Keyword%'  order by id desc " ;}
   ...
   if($num>0){
	 ...
	 while($result=$GLOBALS["db"]->fetch_assoc($row)){
		...
		// echo "<td>".$title."</td>" ;
		 ...
		 echo "<td><a href='MessageEdit.php?Result=Modify&ID=".$id."' onClick=changeAdminFlag('查看留言信息')>查看</a>&nbsp;&nbsp;&nbsp;&nbsp;<input name='selectID[]' type='checkbox' value='".$id."' style='HEIGHT: 13px;WIDTH: 13px;'></td>" ;
		 echo "</tr>";
	     $i++;
	   }
	   echo "<tr><tD colspan='4'></td><Td><input name='submitDelSelect' type='button' class='button'  id='submitDelSelect' value='删除所选' onClick=ConfirmDel('您真的要删除这些留言吗？');></td></tr>" ;
	}else{
	  echo "<tr><tD colspan='5'>暂无信息</td></tr>";
	}
