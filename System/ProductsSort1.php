...
switch($Action){
  ...
  Case "Del" ;
	...
	if($yes){
       ...
       echo "<script language=javascript> alert('成功删除本类、子类及所有下属信息条目，点击确定查看类别树！');location.replace('ProductsSort1.php');</script>" ;
	}	
	break;
  Case "Save" ;
	saveFolder () ;
...
<table width="100%" border="0" cellpadding="3" cellspacing="1" class="input">
  ...
  <tr>
    <td height="24" align="center" nowrap   ><!--<a href="ProductsSort1.php?Action=Add&ParentID=0">添加一级分类</a>--></td>
  </tr>
  <tr>
...
//'列出所有节点------------------------------
Function Folder($id){
  $sql="Select * From jp_category where parentid=$id and sortpath like '%0,14,%'  order by px asc " ;
  $row=mysql_query($sql);
  ...
  while($result=mysql_fetch_array($row)){
	  ...
	  $parentid==0?$FolderName="<strong>$sortname</strong>":$FolderName="<font color='#FF0000'>".$px."</font>&nbsp;".$sortname ."<font color='#FF0000'>(".$sortname_en.")</font>" ;
	...
	if($parentid==0){
       echo "<font color='#FF0000'>分类：</font><a href='ProductsSort1.php?Action=Add&ParentID=$id'>添加</a>" ;
	  // echo "<font color='#367BDA'>&nbsp;&nbsp;</font><a href='ProductsSort1.php?Action=Edit&ID=$id'>修改</a>" ;
	  // echo "<font color='#367BDA'>&nbsp;|&nbsp;</font><a href='ProductsSort1.php?Action=Del&ID=$id'>删除</a>" ; 
	}else{
	 //  echo "<font color='#FF0000'>分类：</font><a href='ProductsSort1.php?Action=Add&ParentID=$id'>添加</a>" ;
	   echo "<font color='#367BDA'>&nbsp;&nbsp;</font><a href='ProductsSort1.php?Action=Edit&ID=$id'>修改</a>" ;
       echo "<font color='#367BDA'>&nbsp;|&nbsp;</font><a href='ProductsSort1.php?Action=Del&ID=$id'>删除</a>" ; 
...
?>
<table width="100%" border="0" cellpadding="3" cellspacing="1" class="input">
<form name="FolderForm" method="post" action="ProductsSort1.php?Action=Save&From=Add">
...
Function saveFolder(){
  ...
  echo "<script language=javascript> alert('".$Action."保存成功，点击确定查看类别树！');location.replace('ProductsSort1.php');</script>" ;
}
...
?>
<table width="100%" border="0" cellpadding="3" cellspacing="1"  class="input">
<form name="FolderForm" method="post" action="ProductsSort1.php?Action=Save&From=Edit&ID=<?php echo $id ?>">
  <input readonly name="ParentID" type="hidden" class="textfield" id="ParentID" size="6" value="<?php echo $parentid ?>">
