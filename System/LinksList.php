    ...
    <td height="36" align="left" nowrap  bgcolor="#EBF2F9"  style="padding-left:20px;"><table width="86%" border="0" cellspacing="0">
      <tr>
        <form name="formSearch" method="get" action="?Result=Search">
          <td width="52%" nowrap> 链接检索：关键字：<input name="Keyword" type="text" class="textfield" value="<?php echo  $Keyword ?>" size="10">
          <input name="submitSearch" type="submit" class="button" value="检索">
          </td>
        </form>
        <Td align="right"><A href="LinksEdit.php?Result=Add">添加链接</A></Td>
      </tr>
    </table>      </td>    
  </tr>
</table>
...
<table width="93%" border="0" cellpadding="3" cellspacing="1" class="input">
  <form action="DelContent.php?Result=jp_link" method="post" name="formDel" >
    <tr >
      ...
      <td width="60%"  ><strong>链接标题</strong> </td>
...
function NewsList($Keyword){
   ...
   $sql="select * from jp_link where sortpath like '%0,14,%' order by id desc " ;
   if($Keyword){
      $sql="select * from jp_link where sortpath like '%0,14,%' and link_name like '%$Keyword%'  order by id desc " ;
   }
   ...
   if($num>0){
	 ...
	 while($result=$GLOBALS["db"]->fetch_assoc($row)){
		 ...
		 echo "<td><a href='LinksEdit.php?Result=Modify&ID=".$id."' onClick=changeAdminFlag('修改链接信息')>修改</a>&nbsp;&nbsp;&nbsp;&nbsp;<input name='selectID[]' type='checkbox' value='".$id."' style='HEIGHT: 13px;WIDTH: 13px;'></td>" ;
		 echo "</tr>";
	     $i++;
	   }
	   echo "<tr><tD colspan='3'></td><Td><input name='submitDelSelect' type='button' class='button'  id='submitDelSelect' value='删除所选' onClick=ConfirmDel('您真的要删除这些链接吗？');></td></tr>" ;
	}else{
