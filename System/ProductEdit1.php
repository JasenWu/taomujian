...
<script language="javascript" >
 function check_add(){
    if(document.editForm.NewsNameSi.value==""){alert("请填写信息标题");document.editForm.NewsNameSi.focus();return false;}
	if(document.editForm.SortID.value==""){alert("请选择信息类别");return false;}
	return true;
 }
</script>
...
NewsEdit($ID) ;
if(!isset($px)){
  $px=$db->getOne("select max(px) as orders from jp_product where sortpath like '%0,20,%' ");
  $px++;	
}
...
?>
 <form name="editForm" method="post" action="ProductEdit1.php?Action=SaveEdit&Result=<?php echo $Result ?>&ID=<?php echo $ID ?>"  onSubmit="return check_add();" >
...
<table width="100%"  border="1" style="border:#6298E1 tthin solid;" cellpadding="3" cellspacing="1" class="input">
    ...
    <tr>
      ...
      <table width="100%" border="0" cellpadding="0" cellspacing="0"   idth="100%">
          ...
          <tr>
            <td height="20" align="right">信息名称(中)：</td>
          ...
          <tr>
            <td height="20" align="right"><strong>信息名称(英)</strong>：</td>
            <td><input name="NewsNameEn" type="text" class="textfield" id="NewsNameEn" style="WIDTH: 240;" value="<?php echo $NewsNameEn ?>" maxlength="100"></td>
          </tr>
          <tr>
            <td height="20" align="right"> CAS(中)：</td>
            <td><input name="video" type="text" class="textfield" id="video" style="WIDTH: 240;" value="<?php echo isset($video)?$video:"" ?>" maxlength="100"></td>
          </tr>
          <tr>
            <td height="20" align="right"><strong> CAS  (英)</strong>：</td>
            <td><input name="video1" type="text" class="textfield" id="video1" style="WIDTH: 240;" value="<?php echo isset($video1)?$video1:"" ?>" maxlength="100"></td>
          </tr>
          <tr>
            <td height="20" align="right">信息图片：</td>
            <td><input name="type1" type="text" class="textfield" id="type1" style="WIDTH: 240;" value="<?php echo $type1 ?>" maxlength="100"><input type="button" value="浏览图片" onClick="BrowseServer( 'Images:/', 'type1' );" />(156px*156px)</td>
          </tr>
          <tr>
          </tr>
          <tr class="sort" >
            <td height="20" align="right"> 所属类别：</td>
            <td><?php  show_selects(14,@$SortIDs[0]) ;?>
               
            </td>
          </tr>
          <tr style="display:none">
            <td height="20" align="right">主要信息推荐：</td>
            <td colspan="5"><input name="ViewFlagSi" type="checkbox" style='HEIGHT: 13px;WIDTH: 13px;' value="1" <?php if($ViewFlagSi){ echo "checked" ;} ?> ></td>
          </tr>
          <tr style="display:none">
            <td height="20" align="right" valign="top">信息介绍(中)：<br>
          ...
          <tr style="display:none">
            <td height="20" align="right" valign="top"><strong>信息介绍(英)</strong>：<br>
		  ...
		  <?php
		   ...
		   ?>  maxlength="100"></td>
                ...
                <table width="100%" border="0" cellpadding="0" cellspacing="0"   idth="100%">
                  <tr >
                        <td height="20" align="right" width="120">中文关键字：</td>
                 ...
                 <tr >
                  ...
                  <tr>
                    <td height="20" align="right" valign="top"><strong>英文描述</strong>：
...
function NewsEdit($ID){
   ...
   if($Action=="SaveEdit"){  //添加修改数据
	 ...
	 $NewsNameEn=$_POST["NewsNameEn"] ;
     $SortID=$_POST["SortID"] ;
     $ContentSi=escape_character($_POST["ContentSi"]) ;
	 $ContentEn=escape_character($_POST["ContentEn"]) ;
     $AddTime=strtotime($_POST["AddTime"]) ;
	 $type1=$_POST["type1"] ;
	 
	 $keyword=escape_character($_POST["keyword"]) ;
	 $description=escape_character($_POST["description"]) ;
	 $keyword_en=escape_character($_POST["keyword_en"]) ;
	 $description_en=escape_character($_POST["description_en"]) ;
	 $video=escape_character($_POST["video"]) ;
	 $video1=escape_character($_POST["video1"]) ;
	 $SortPath="";
	 ...
	 if(!empty($SortID)){
		 ...
		 echo "<script>alert('情选择信息类别！');history.back();</script>";
	 }
	 
	 @$_POST["ViewFlagSi"]==1?$ViewFlagSi=1:$ViewFlagSi=0;
	 $data_fields=" set `px`='$px',  `product_content`='$ContentSi',  `type3`='$ContentEn' ,`product_name`='$NewsNameSi',`type2`='$NewsNameEn'  , `add_time`=$AddTime  , `sortid`='$SortID', `sortpath`='$SortPath', `type1`='$type1', `viewflag`='$ViewFlagSi', `keyword`='$keyword', `description`='$description', `keyword_en`='$keyword_en', `description_en`='$description_en',video='$video',video1='$video1' " ; 
	// echo  $data_fields;
     if($Result=="Add"){
	 ...
	 echo "<script>alert('成功编辑信息信息！');location.href='ProductList1.php';</script>";
  }else{   //提取信息
     if($Result=="Modify"){
