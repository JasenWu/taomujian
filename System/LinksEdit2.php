...
<script language="javascript" >
 function check_add(){
    if(document.editForm.link_name.value==""){alert("请填写荣誉名称");document.editForm.link_name.focus();return false;}
	return true;
 }
</script>
...
<table width="100%"  border="1" style="border:#6298E1 tthin solid;" cellpadding="3" cellspacing="1" class="input">
  <form name="editForm" method="post" action="LinksEdit2.php?Action=SaveEdit&Result=<?php echo $Result ?>&ID=<?php echo $ID ?>"  onSubmit="return check_add();" >
    <tr>
...
function NewsEdit($ID){
   ...
   if($Action=="SaveEdit"){  //添加修改数据
	 ...
	 $data_fields=" set `link_name`='$link_name' , `sortid`='$SortID', `sortpath`='$SortPath'  , `add_time`=$AddTime , `px`='$px',`type1`='$type1',`type2`='$type2' " ; 
     if($Result=="Add"){
	 ...
	 echo "<script>alert('成功编辑荣誉信息！');location.href='LinksList2.php';</script>";
  }else{   //提取信息
     if($Result=="Modify"){
