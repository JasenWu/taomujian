...
<script language="javascript" >
 function check_add(){
    if(document.editForm.NewsNameSi.value==""){alert("请填写视频标题");document.editForm.NewsNameSi.focus();return false;}
	if(document.editForm.SortID.value==""){alert("请选择视频类别");return false;}
	return true;
 }
</script>
...
NewsEdit($ID) ;
if(!isset($px)){
  $px=$db->getOne("select max(px) as orders from jp_product where sortpath like '%0,14,%' ");
  $px++;	
}
...
<table width="100%"  border="1" style="border:#6298E1 tthin solid;" cellpadding="3" cellspacing="1" class="input">
  <form name="editForm" method="post" action="ProductEdit3.php?Action=SaveEdit&Result=<?php echo $Result ?>&ID=<?php echo $ID ?>"  onSubmit="return check_add();" >
    <tr>
      <td height="24" nowrap ><table width="100%" border="0" cellpadding="0" cellspacing="0"   idth="100%">
          ...
          <tr>
            ...
            <td><input name="px" type="text" class="textfield" id="px" style="WIDTH: 240;" value="<?php echo $px ?>" maxlength="100">(降序排序，数字大的排前面)</td>
          </tr>
          <tr>
            <td height="20" align="right">视频名称：</td>
          ...
          <tr >
           ...
           <tr>
            <td height="20" align="right">视频图片：</td>
          ...
          <tr>
            <td height="20" align="right" valign="top">视频路径：<br>
            <td>
               <input name="ContentSi" type="text" class="textfield" id="ContentSi" style="WIDTH: 240;" value="<?php echo $ContentSi ?>" maxlength="100"></td>
          </tr>
...
function NewsEdit($ID){
   ...
   if($Action=="SaveEdit"){  //添加修改数据
	 ...
	 vido_list("../video/playlist".$ID.".xml",$NewsNameSi,$description,$ContentSi);
	 echo "<script>alert('成功编辑视频信息！');location.href='ProductList3.php';</script>";
  }else{   //提取信息
     if($Result=="Modify"){
	    ...
	    @$yz=is_array($row=$GLOBALS["db"]->fetch_assoc($result));
		 if($yz){
		     global $px,$NewsNameSi,$NewsNameEn,$SortIDs,$SortPath,$ViewFlagSi,$ContentSi,$ContentEn,$AddTime,$Author,$type1,$keyword,$description,$keyword_en,$description_en;
			 $px=$row["px"] ;
...
function vido_list($url,$title,$desc,$files)
{
 $info='<?xml version="1.0" encoding="utf-8"?>';
 $info.='<playlist version="1" xmlns="http://xspf.org/ns/0/">';
 $info.='<trackList>';
 $info.='<track>';
 $info.='<title>'.$title.'</title>';
 $info.='<creator>'.$desc.'</creator>';
 $info.='<location>'.$files.'</location>';
 $info.='</track>';
 $info.='</trackList>';
 $info.='</playlist>';
 $f=fopen($url,"w");
...
<playlist version="1" xmlns="http://xspf.org/ns/0/">
	<trackList>
		...
		<track>
			<title>高温摄像机实况</title>
			<creator>描述</creator>
			<location>gaowen.flv</location>
		</track>
		
		<track>
			<title>东震机清介绍</title>
			<creator>描述</creator>
			<location>jiqing.flv</location>
		</track>
