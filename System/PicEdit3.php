...
NewsEdit($ID) ;
if(!isset($px)){
  $px=$db->getOne("select max(views) as orders from jp_news where sortpath like '%0,3,%' ");
  $px++;	
}
...
<table width="100%"  border="1" style="border:#6298E1 tthin solid;" cellpadding="3" cellspacing="1" class="input">
  <form name="editForm" method="post" action="PicEdit3.php?Action=SaveEdit&Result=<?php echo $Result ?>&ID=<?php echo $ID ?>"  onSubmit="return check_add();" >
    <tr>
      <td height="24" nowrap ><table width="100%" border="0" cellpadding="0" cellspacing="0"   idth="100%">
          ...
          <tr>
            <td height="20" align="right">banner名称(中)：</td>
          ...
          <tr>
            <td height="20" align="right">banner名称(英)：</td>
          ...
          <tr style="display:none;">
            <td height="20" align="right">连接地址：</td>
          ...
          <tr>
            ...
            <td><?php  show_select(45,$SortIDs) ;?>
            </td>
          </tr>
          <tr>
            <td height="20" align="right">banner flash：</td>
            <td><input name="type1" type="text" class="textfield" id="type1" style="WIDTH: 240;" value="<?php echo $type1 ?>" maxlength="100"><input type="button" value="浏览flash" onClick="BrowseServer( 'Flash:/', 'type1' );" /></td>
          </tr>
          <tr>
            <td height="20" align="right">banner类型：</td>
            <td colspan="5">
...
function NewsEdit($ID){
   ...
   if($Action=="SaveEdit"){  //添加修改数据
	 ...
	 $data_fields=" set `news_name`='$NewsNameSi',`news_content`='$news_content'  , `add_time`=$AddTime ,`sortid`='$SortID', `sortpath`='$SortPath', `type2`='$type2', `type1`='$type1', `author`='$author', `views`='$views' " ; 
     if($Result=="Add"){
	 ...
	 echo "<script>alert('成功编辑banner信息！');location.href='PicList3.php';</script>";
  }else{   //提取信息
     if($Result=="Modify"){
	    ...
	    @$yz=is_array($row=$GLOBALS["db"]->fetch_assoc($result));
		 if($yz){
		     global $NewsNameSi,$news_content,$SortIDs,$SortPath,$ViewFlagSi,$ContentSi,$AddTime,$type2,$type1,$author,$px;
