      ...
      <table width="86%" border="0" cellspacing="0">
       <tr>
        <form name="formSearch" method="get" action="?Result=Search">
         ...
         <Td style="text-align:right; padding-right:100px;"><a href="MemberEdit.php?Result=Add">添加帐号</a></Td>
       </tr>
     </table>     
...
<table width="93%" border="0" cellpadding="3" cellspacing="1" class="input">
  <form action="DelContent.php?Result=jp_member" method="post" name="formDel" >
    <tr >
...
function NewsList($Keyword){
   ...
   $sql="select * from jp_member order by id desc " ;
   if($Keyword){
      $sql="select * from jp_member where adminname like '%$Keyword%'  order by id desc " ;
   }
   ...
   if($num>0){
	 ...
	 while($result=$GLOBALS["db"]->fetch_assoc($row)){
		 ...
		 echo "<td>$i</td>" ;
	     echo "<td>$cname</td>" ;
		 echo "<td>".date('Y-m-d',$addtime)."</td>" ;
		 echo "<td><a href='MemberEdit.php?Result=Modify&ID=".$id."' onClick=changeAdminFlag('修改新闻信息')>修改</a>&nbsp;&nbsp;&nbsp;&nbsp;<input name='selectID[]' type='checkbox' value='".$id."' style='HEIGHT: 13px;WIDTH: 13px;'></td>" ;
		 echo "</tr>";
	     $i++;
	   }
