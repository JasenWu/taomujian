    ...
    <td height="36" align="left" nowrap  bgcolor="#EBF2F9"  style="padding-left:20px;"><table width="86%" border="0" cellspacing="0">
      <tr>
        <form name="formSearch" method="get" action="?Result=Search">
          <td width="52%" nowrap> 职位检索：关键字：<input name="Keyword" type="text" class="textfield" value="<?php echo  $Keyword ?>" size="10">
          <input name="submitSearch" type="submit" class="button" value="检索">
          </td>
        </form>
        <Td ><A href="JobsEdit.php?Result=Add"><strong style="color:#F00">添加职位</strong></A></Td>
      </tr>
    </table>      </td>    
  </tr>
</table>
...
<table width="93%" border="0" cellpadding="3" cellspacing="1" class="input">
  <form action="DelContent.php?Result=jp_job" method="post" name="formDel" >
    <tr >
      <td width="55" nowrap ><strong>ID</strong> </td>
      <td width="60%"  ><strong>职位标题</strong> </td>
      <td width="95" nowrap ><strong>职位类别</strong></td>
...
function NewsList($Keyword){
   ...
   $sql="select * from jp_job  order by id desc " ;
   if($Keyword){
      $sql="select * from jp_job where job_name like '%$Keyword%' or job_content like '%$Keyword%' or type3 like '%$Keyword%'  order by id desc " ;
   }
   ...
   if($num>0){
	 ...
	 while($result=$GLOBALS["db"]->fetch_assoc($row)){
		 ...
		 echo "<td><a href='JobsEdit.php?Result=Modify&ID=".$id."' onClick=changeAdminFlag('修改职位信息')>修改</a>&nbsp;&nbsp;&nbsp;&nbsp;<input name='selectID[]' type='checkbox' value='".$id."' style='HEIGHT: 13px;WIDTH: 13px;'></td>" ;
		 echo "</tr>";
	     $i++;
	   }
	   echo "<tr><tD colspan='4'></td><Td><input name='submitDelSelect' type='button' class='button'  id='submitDelSelect' value='删除所选' onClick=ConfirmDel('您真的要删除这些职位吗？');></td></tr>" ;
	}else{
