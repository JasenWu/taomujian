...
KE.event.ready(function() {
	...
	var insertImage = function(url, title) {
		if (!insertLink(url)) {
			KE.util.insertHtml(id, '<img src="../../../kind_editor_lxw/plugins/file_manager/' + url + '" alt="' + title + '" border="0" />');
		}
	};
	var insertFile = function(url, title) {
		if (!insertLink(url)) {
			KE.util.insertHtml(id, '<a href="../../../kind_editor_lxw/plugins/file_manager/' + url + '" target="_blank">' + title + '</a>');
		}
	};
