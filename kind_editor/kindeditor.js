...
KE.util = {
	...
	getFullHtml : function(id) {
		...
		html += '<link href="../kind_editor_lxw/' + KE.g[id].skinsPath + 'common/editor.css?ver=' + escape(KE.version) + '" rel="stylesheet" type="text/css" />';
		var cssPath = KE.g[id].cssPath;
		if (typeof cssPath == 'string') cssPath = [cssPath];
		for (var i = 0, len = cssPath.length; i < len; i++) {
			if (cssPath[i] !== '') html += '<link href="../kind_editor_lxw/' + cssPath[i] + '" rel="stylesheet" type="text/css" />';
		}
		html += '</head>';
...
KE.plugin['emoticons'] = {
	...
	exec : function(id, num) {
		...
		var html = '<img src="../kind_editor_lxw/' + src + '" kesrc="' + src + '" border="0" alt="" />';
		KE.util.insertHtml(id, html);
		this.menu.hide();
		KE.util.focus(id);
	}
...
KE.plugin['image'] = {
	...
	insert : function(id, url, title, width, height, border, align) {
		var html = '<img src="../kind_editor_lxw/' + url + '" kesrc="' + url + '" ';
