...
$smarty->template_dir = FILE_PATH."/cn/templates";                      //设置模板目录
$smarty->compile_dir = FILE_PATH."/cn/templates_c";                     //设置编译目录
$smarty->cache_dir = FILE_PATH."/cn/cache";                             //缓存文件夹
...
function contents($types){
	$sql="select * from kaisi_article where m_id=$types";
	$result=$GLOBALS["db"]->getAll($sql);
	$GLOBALS["smarty"]->assign("contents",$result);
}

function product_lists($types=1){ //下来显示信息的类别
  $sql="select sortname,id,contentsi from kaisi_category where parentid=$types  order by px asc,id asc  " ;
  $result=$GLOBALS["db"]->getAll($sql);
  $GLOBALS["smarty"]->assign("product_lists",$result);
}

function show_banner($sid,$types=0){
	$sql="select type1 from kaisi_news where sortid=".$sid." and type2 like '%$types%' limit 1 " ;
...
{
	$sql="select type1,author,news_name from kaisi_news where type2 like '%0,%' and sortid='".$type."'  order by views asc ,id desc ";
...
function keywords($id){
   $sql="select * from kaisi_article where m_id='".$id."' ";
...
function tj_news()
{
	$sql="select news_name,id from kaisi_news where viewflag=1 and sortpath like '%0,1,2,%' order by add_time desc,id desc";
	$results=$GLOBALS["db"]->getAll($sql);
...
{
    $sql="select sortname,id from kaisi_menu_sort where parentid='".$id."' ". $dataother ." order by px asc ,id desc ";
...
{
	 $sql="select sortname,id  from kaisi_category where parentid='".$id."' order by px asc,id asc  ";
	 $result=$GLOBALS["db"]->getAll($sql);
...
function banner_pic($id){
  $sql="select type1 from kaisi_news where sortid=".$id." and type2 like '%0%' limit 1 ";
  $result=$GLOBALS["db"]->getAll($sql) ;
  $GLOBALS["smarty"]->assign("pic",$result) ;       //banner图片	
}

//新闻信息
function  news_show($id,$data_base="kaisi_category"){
	...
	foreach($result as $value){
		 ...
		 $result_small=$GLOBALS["db"]->getAll("select news_name,id,add_time from kaisi_news where sortid=".$value["id"]." order by add_time desc,id desc limit 5 ");
		 $j=0;
		 if($result_small){
...
function sort_list_news($id,$data_base="kaisi_menu_sort",$ass="about_menu"){
   $ass=="services"?$datewhere=" and id<>20":$datewhere="";
   $ass=="join"?$datewhere=" and id not in(26,27,40)":$datewhere="";
...
function sort_list_en($id,$data_base="kaisi_menu_sort",$ass="about_menu"){
   $ass=="services"?$datewhere=" and id<>20":$datewhere="";
   $ass=="join"?$datewhere=" and a.id not in(26,27,40)":$datewhere="";
   $sql="select a.id,b.sortname from ".$data_base." as a ,kaisi_article as b where parentid=".$id."  ".$datewhere." and a.id=b.m_id order by px asc " ;
   $result=$GLOBALS["db"]->getAll($sql);
   $GLOBALS["smarty"]->assign($ass,$result) ;
}
...
function sort_name($id,$fileds="sortname"){
  if(is_numeric($id)){
	 $sort_names=$GLOBALS["db"]->getOne("select ".$fileds." from kaisi_category where id=".$id);
	 return $sort_names;  
  }	
}
//多信息类别名称
function sort_name_more($id,$files="sortname,sortname_en",$find_id="id"){
  if(is_numeric($id)){
	 $sort_names=$GLOBALS["db"]->getAll("select ".$files." from kaisi_category where ".$find_id."=".$id);
	 return $sort_names;  
  }	
}
...
//信息列表(带分页)
function information_list($url,$pageSize,$field,$data_where="",$order="add_time desc,id desc",$date_base="kaisi_news",$assign_name="info_reslut"){
...
function case_countent($id){
	if(is_numeric($id)){
	  $result=$GLOBALS["db"]->getOne("select news_content from kaisi_news where id=".$id);
	  if(!empty($result)){$result=str_replace("../Uploadfiles/","/Uploadfiles/",$result);}
	  $navi=$GLOBALS["db"]->getOne("select news_name from kaisi_news where id=".$id);
...
function about_countent($id,$cut_word="",$ass="about_content",$navis="navi"){
	if(is_numeric($id)){
	  ...
	  if(!empty($result)){$result=str_replace("../Uploadfiles/",SERVER_NAME."/Uploadfiles/",$result);}
	  if(!empty($cut_word)){
		  if($result){
	  ...
	  $navi=$GLOBALS["db"]->getAll("select a.sortname,a.keyword,a.description,a.content ,b.sortname as  sortname_en from kaisi_article as a,kaisi_menu_sort as b where a.m_id=b.id and a.m_id=".$id);
...
function product_list_info(){
	$sql="select sortname_en as sortname,id from kaisi_category where parentid=27 order by px asc ";
	$result=$GLOBALS["db"]->getAll($sql);
	$i=0;
	foreach($result as $value)
	{
		 $lists[$i]["info_id"]=$value["id"];
		 $lists[$i]["info_name"]=$value["sortname"];
		 $result_small=$GLOBALS["db"]->getAll("select sortname_en as sortname,id from kaisi_category where parentid=".$value["id"]." order by px asc ");
		 $j=0;
		 if($result_small)
		 {
			 foreach($result_small as $va){
				...
				$result_details=$GLOBALS["db"]->getAll("select type2 as product_name,id from kaisi_product where sortid=".$va["id"]."  order by px asc,id desc");
				$k=0;
				if($result_details){
			 ...
			 $lists[$i]["posts"]="";
			 $result_d=$GLOBALS["db"]->getAll("select type2 as product_name,id from kaisi_product where sortid=".$value["id"]."  order by px asc,id desc");
			 $x=0;
			 if($result_d){
...
function sort_info($id){
	$sid=$GLOBALS["db"]->getOne("select sortid from kaisi_product where id=".$id);
	$result_s=$GLOBALS["db"]->getAll("select sortname_en as sortname,id,	parentid from kaisi_category where id=".$sid);
	$result_s[0]["url"]="products_one.php";
    if($result_s){
	    if($result_s[0]["parentid"]!=27){
		  $result_b=$GLOBALS["db"]->getAll("select sortname_en as sortname,id,parentid from kaisi_category where id=".$result_s[0]["parentid"]);
...
function home_news($types,$pic_news,$other_news){
	$sql="select news_name,id,news_content,type1 from kaisi_news where sortid=".$types." and type1<>''  and viewflag=1 order by viewflag desc,add_time desc limit 1 ";
    $pic_result=$GLOBALS["db"]->getAll($sql);
	@$f_id=$pic_result[0]["id"];
	if(is_numeric($f_id)){
		$sql1="select news_name,id,add_time from kaisi_news where sortid=".$types." and id<>".$f_id." order by viewflag desc,add_time desc limit 5 ";
	}else{
	    $sql1="select news_name,id,add_time from kaisi_news where sortid=".$types." order by viewflag desc,add_time desc limit 9 ";
		$pic_result="";
	}
	//echo $sql1."<br>";
...
function home_tj_product(){
   $sql="select type2,id from kaisi_product order by viewflag desc ,px asc,id desc limit 6 " ;
   $result=$GLOBALS["db"]->getAll($sql);
   $GLOBALS["smarty"]->assign("tj_pro",$result);
}

function show_select($types){ //下来显示信息的类别
  $sql="select sortname,id from kaisi_category where parentid=$types and isDel=1 order by px asc,id asc  " ;
  $result=$GLOBALS["db"]->getAll($sql);
  $lists="";
  foreach($result as $v){
	  extract($v);
...
function child_sort($p_id){
	$listss="";
	$sql="select sortname,id from kaisi_category where parentid=$p_id  order by px asc,id asc  " ;
	$result=$GLOBALS["db"]->getAll($sql);
	if(!empty($result)){
		$listss.= "<ul>";
		foreach($result as $v){
